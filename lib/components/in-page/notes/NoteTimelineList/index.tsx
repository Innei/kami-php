...
> = (props) => {
  ...
  return (
    <div className={cn(styles['container'], className)} data-hide-print>
      <div className={cn(styles.list)}>
        ...
        {note?.topic && (
          <>
            ...
            <p className="flex min-w-0 flex-col overflow-hidden text-theme-gray-1">
              此文章收录于专栏：
              <br />
              <FloatPopover
                placement="right"
  ...
  (props) => {
    ...
    return (
      <li className="flex items-center">
        <LeftToRightTransitionView in={active}>
          <i className="icon-[material-symbols--arrow-circle-right-outline-rounded] text-always-pink-600" />
        </LeftToRightTransitionView>
        <Link
          shallow
          className={cn(active ? styles['active'] : null, styles.item)}
          href={`/notes/${item.nid}`}
          key={item.id}
          scroll={false}
