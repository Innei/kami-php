...
import { ImageLazy, calculateDimensions } from '~/components/ui/Image'
...
export const Gallery: FC<GalleryProps> = (props) => {
  ...
  return (
    <div
      className={cn('w-full', 'relative', styles['root'])}
      ref={ref}
    ...
    >
      ...
      >
        {images.map((image) => {
          ...
          return (
            <div
              style={childStyle}
              className={cn(styles['child'], 'inline-block')}
      ...
      <div className={cn(styles['indicator'], 'space-x-2')}>
        {Array.from({
          ...
          return (
            <div
              className={cn(
                'h-[6px] w-[6px] cursor-pointer rounded-full bg-dark-50 opacity-50 transition-opacity duration-200 ease-in-out',
