...
const _CommentWrap: FC<CommentWrapProps> = observer((props) => {
  ...
  const fetchComments = useCallback(
    (page = 1, size = 10) => {
      RESTManager.api.comments
        .ref(id)
        .get<{ data: CommentModel[]; page: PagerModel['page'] }>({
          params: {
            page,
            size,
            ts: new Date().getTime(),
        ...
        .then(({ data, page }) => {
          collection.clear()
          flattenChildren(data).forEach((i: any) => {
  ...
  const handleComment = async (model) => {
    openCommentMessage()
    if (logged) {
      await RESTManager.api.comments.master.comment(id).post({
        params: {
          ref: type,
          ts: new Date().getTime(),
        },
      ...
      await RESTManager.api.comments.comment(id).post({
        params: {
          ref: type,
          ts: new Date().getTime(),
        },
