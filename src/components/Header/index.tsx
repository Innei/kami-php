...
import { useHistory as useRouter, useLocation, useParams } from 'react-router'
import { Link } from 'react-router-dom'
import { useStore } from 'store'
import { appStore } from 'store/app.store'
import { isLikedBefore, NoSSR, RESTManager, setLikeId } from 'utils'
...
export const _Header: FC = observer(() => {
  ...
  const { app: appStore, ui: uiStore, user: userStore } = useStore()
      ...
      <header
        className={classNames(
          ...
          !appStore.headerNav.show &&
            uiStore.isOverFirstScreenHeight &&
            uiStore.viewport.mobile
            ? styles['hide']
            : null,
        )}
      >
        <nav
          className={classNames(
            styles['nav-container'],
            appStore.headerNav.show &&
              (uiStore.scrollDirection == 'down' || uiStore.viewport.mobile) &&
              uiStore.isOverPostTitleHeight
              ? styles['toggle']
              : null,
          )}
        >
